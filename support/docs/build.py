...
def generate_C_API_rst(project):
    ...
    outFile = open(project.dir_docs_build_src + "/" + filename_root + '.rst', "w")
...
def generate_Python_API_rst(project):
    ...
    outFile.write(".. automodule:: "+project.name+"\n")
    outFile.write("    :members:\n")
    ...
    package = __import__(project.name)
...
def generate_index_rst(project):
    ...
    for doc_i in doc_order:
        ...
        if(os.path.isfile(os.path.join(project.dir_docs,doc_i))):
            outFile.write("   "+doc_i+"\n")
...
def generate_project_rsts(project):
    ...
    if(project.is_Python_project):
        ...
        subprocess.check_call(["sphinx-apidoc","-o"+project.dir_docs_build_src,project.dir_python_pkg,"-T","-e","-M","-f"])

        # Remove the unneeded 'gbpPy.rst' file generated by 'sphinx-apidoc'
        os.remove(os.path.join(project.dir_docs_build_src,project.name+".rst"))
