...
def package_files(directory='gbpBuild_dev/data'):
    """
...
if os.path.exists(os.path.join(dir_root, '.git')):
    ...
    if git_version_string not in tags:
        ...
        print(cmd)
        # ... or, replace the previous lines with the following commented-out line to do this automatically
...
setup(
    ...
    package_data={
        'gbpBuild_dev': package_files()},
    entry_points={
        'console_scripts': [
            'update_gbpBuild_docs=%s.scripts.update_gbpBuild_docs:update_gbpBuild_docs' %
            (package_name)]},
