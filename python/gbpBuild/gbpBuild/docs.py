...
def generate_C_execs_rst(project):
    ...
    for module_i in module_list:
        ...
        for exec_i in exe_list:
            ...
            out = subprocess.check_output([project.params['path_project_root'] + "/build/" + exec_i, "-h"]).decode("utf-8")
...
def generate_Python_API_rst(project):
    ...
    for package_name in project.params['python_packages']:
        package = __import__(package_name)
        submodule_list = list_submodules(package)
        for submodule_i in submodule_list:
            outFile.write("   "+project.params['name']+"." + submodule_i + "\n")
    ...
    if(project.params['is_Python_project']):
        ...
        for python_pkg in project.params['python_packages']:
            ...
            subprocess.check_call(["sphinx-apidoc",
                                   ...
                                   python_pkg_path,
                                   python_pkg_path + "/setup.py",
                                   python_pkg_path + '/' + os.path.basename(python_pkg_path) + '/scripts/',
                                   python_pkg_path + '/' + os.path.basename(python_pkg_path) + '/_internal/',
                                   python_pkg_path + '/' + os.path.basename(python_pkg_path) + '/tests/',
                                   "-T",
                                   "-e",
                                   "-M",
                                   "-f"])
