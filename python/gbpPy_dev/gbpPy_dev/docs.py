...
def generate_C_execs_rst(project):
    ...
    for module_i in module_list:
        ...
        for exec_i in exe_list:
            ...
            out = subprocess.check_output([project.dir_root+"/build-dev/"+exec_i, "-h"]).decode("utf-8")
...
def generate_Python_API_rst(project):
    ...
    package = __import__(project.params['name'])
...
def generate_index_rst(project):
    ...
    if(project.params['is_C_project'] and project.params['is_Python_project']):
        doc_order.append('api/C_execs.rst')
        doc_order.append('api/Python_execs.rst')
        doc_order.append('api/C_API.rst')
        doc_order.append('api/Python_API.rst')
...
def generate_project_rsts(project):
    ...
    if(project.params['is_Python_project']):
        ...
        subprocess.check_call(["sphinx-apidoc","-o"+project.params['dir_docs_api_src'],project.params['dir_python_pkg'],"-T","-e","-M","-f"])
